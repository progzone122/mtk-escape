...
from carbonara_checker import parse_da_loader
...
def add_device(
    ...
    device_entry = vendor_data.get(
        codename, {"name": name, "da_files": None, "auth_file": None}
    )
    ...
    if da_path:
        ...
        existing_da = next((da for da in da_list if da["path"] == da_path), None)

        if carbonara:
            carbonara_status = True
        else:
            carbonara_status = parse_da_loader(da_path) or False

        if existing_da:
            existing_da["flags"]["carbonara"] = carbonara_status
            existing_da["flags"]["only_stage1"] = only_stage1
        else:
            da_list.append(
                {
                    "path": da_path,
                    "flags": {
                        "carbonara": carbonara_status,
...
def main():
    parser = argparse.ArgumentParser(
        description="Add or update a device in JSON config"
    )
    parser.add_argument(
        "--config", "-c", default="repo.json", help="Path to JSON config file"
    )
    parser.add_argument(
        "--vendor", "-v", required=True, help="Vendor name (e.g., motorola, huawei)"
    )
    ...
    parser.add_argument(
        "--carbonara", action="store_true", help="Carbonara flag for the DA file"
    )
    parser.add_argument(
        "--only_stage1", action="store_true", help="Only_stage1 flag for the DA file"
    )
